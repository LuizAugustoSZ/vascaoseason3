...
if ($search !== '') { $where[] = '(conta_nome LIKE ? OR descricao LIKE ? OR recurso_id LIKE ?)'; $like = "%$search%"; array_push($params, $like, $like, $like); }
...
$daily->execute([$days]); $dailyRows = $daily->fetchAll();
...
body{background:#08090b}.dash{padding:42px 0 70px}.dash-top,.metric-grid,.chart-grid{display:grid;gap:16px}.dash-top{grid-template-columns:1fr auto;align-items:end}.metric-grid{grid-template-columns:repeat(4,1fr);margin:24px 0}.metric,.chart-card,.log-panel{background:#111318;border:1px solid #292d35;border-radius:16px}.metric{padding:20px}.metric small{color:#9299a5;text-transform:uppercase}.metric strong{display:block;font:800 2.2rem 'Barlow Condensed',sans-serif}.chart-grid{grid-template-columns:2fr 1fr;margin-bottom:24px}.chart-card{padding:20px;min-height:330px}.filters{display:grid;grid-template-columns:120px 1fr 1fr 2fr auto;gap:10px;padding:16px}.event{font-weight:700;text-transform:capitalize}.details{max-width:360px;color:#aeb4bf}.empty{padding:50px;text-align:center;color:#9299a5}@media(max-width:900px){.metric-grid{grid-template-columns:repeat(2,1fr)}.chart-grid{grid-template-columns:1fr}.filters{grid-template-columns:1fr}.dash-top{grid-template-columns:1fr}}
</style></head><body><main class="dash"><div class="container"><div class="dash-top"><div><span class="eyebrow">Admin Master</span><h1 class="display-4 fw-bold mb-0">DASHBOARD</h1><p class="text-secondary mb-0">Atividade, acessos e alterações registradas no sistema.</p></div><div class="d-flex gap-2"><a class="btn btn-outline-danger" href="sorteador.php">Sorteador</a><a class="btn btn-outline-light" href="index.php">Voltar ao Admin</a></div></div>
...
<div class="chart-grid"><section class="chart-card"><h2 class="h5">Atividade por dia</h2><canvas id="daily-chart"></canvas></section><section class="chart-card"><h2 class="h5">Eventos por módulo</h2><canvas id="module-chart"></canvas></section></div>
<section class="log-panel"><form class="filters"><select class="form-select" name="dias"><?php foreach([7,30,60,90] as $d): ?><option value="<?= $d ?>" <?= $days===$d?'selected':'' ?>><?= $d ?> dias</option><?php endforeach; ?></select><select class="form-select" name="evento"><option value="">Todos os eventos</option><?php foreach($events as $item): ?><option <?= $event===$item?'selected':'' ?>><?= e($item) ?></option><?php endforeach; ?></select><select class="form-select" name="modulo"><option value="">Todos os módulos</option><?php foreach($modules as $item): ?><option <?= $module===$item?'selected':'' ?>><?= e($item) ?></option><?php endforeach; ?></select><input class="form-control" name="busca" value="<?= e($search) ?>" placeholder="Buscar usuário, descrição ou registro"><button class="btn btn-danger">Filtrar</button></form><div class="table-responsive"><table class="table table-hover mb-0"><thead><tr><th>Data</th><th>Evento</th><th>Módulo</th><th>Usuário</th><th>Registro</th><th>Detalhes</th></tr></thead><tbody><?php foreach($rows as $row): ?><tr><td class="text-nowrap"><?= e(format_datetime_br($row['criado_em'])) ?></td><td><span class="event"><?= e(str_replace('_',' ',$row['evento'])) ?></span></td><td><?= e($row['modulo']) ?></td><td><?= e($row['conta_nome'] ?: 'Visitante') ?></td><td><?= e(trim(($row['recurso_tipo'] ?: '') . ' #' . ($row['recurso_id'] ?: ''), ' #')) ?: '—' ?></td><td class="details"><?= e($row['descricao']) ?></td></tr><?php endforeach; ?><?php if(!$rows): ?><tr><td colspan="6" class="empty">Nenhum evento encontrado neste período.</td></tr><?php endif; ?></tbody></table></div><div class="d-flex justify-content-between align-items-center p-3"><small class="text-secondary"><?= $total ?> registro(s)</small><div class="btn-group"><?php if($page>1): ?><a class="btn btn-sm btn-outline-light" href="<?= e(dash_url(['pagina'=>$page-1])) ?>">Anterior</a><?php endif; ?><span class="btn btn-sm btn-dark"><?= $page ?>/<?= $pages ?></span><?php if($page<$pages): ?><a class="btn btn-sm btn-outline-light" href="<?= e(dash_url(['pagina'=>$page+1])) ?>">Próxima</a><?php endif; ?></div></div></section></div></main>
...
new Chart(document.getElementById('daily-chart'),{type:'line',data:{labels:daily.map(x=>x.dia),datasets:[{label:'Logins',data:daily.map(x=>+x.logins),borderColor:'#d71920',backgroundColor:'#d7192033',fill:true,tension:.35},{label:'Alterações',data:daily.map(x=>+x.alteracoes),borderColor:'#f3b33d',tension:.35}]},options:{responsive:true,maintainAspectRatio:false}});
