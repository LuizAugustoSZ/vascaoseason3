<section id="tab-noticias" class="tab-pane fade">
  <div class="row g-4">
    ...
    <div class="col-xl-4"><div class="panel"><div class="panel-head"><h3>Postagens</h3><span><?=count($newsAdmin)?> ativas</span></div><div class="admin-news-list">
      <?php foreach($newsAdmin as $item):?><article><h3><?=e($item['titulo'])?></h3><small><?=e(date('d/m/Y H:i',strtotime($item['publicado_em'])))?> • <?=e($item['autor'])?></small><div class="d-flex gap-2 mt-2"><button type="button" class="btn btn-sm btn-outline-light editar-noticia" data-id="<?=$item['id']?>">Editar</button><form method="post"><input type="hidden" name="csrf" value="<?=e(csrf_token())?>"><input type="hidden" name="action" value="desativar_noticia"><input type="hidden" name="noticia_id" value="<?=$item['id']?>"><button class="btn btn-sm btn-outline-danger">Apagar</button></form></div></article><?php endforeach?>
      <?php if(!$newsAdmin):?><div class="empty-state">Nenhuma notícia publicada.</div><?php endif?>
    </div></div></div>
  </div>
  <script id="news-admin-data" type="application/json"><?=json_encode($newsAdmin,JSON_HEX_TAG|JSON_HEX_AMP|JSON_HEX_APOS|JSON_HEX_QUOT|JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES)?></script>
</section>
