...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    try {
        ...
        if ($action === "criar_supercopa") {
            ...
            // Regra automática: em caso de campeão repetido, entra o vice da primeira origem.
            $sameChampionRule = 'vice_origem_a';
            $teamA = competition_champion_id($pdo, $sourceA);
            $teamB = competition_champion_id($pdo, $sourceB);
            if ($teamA && $teamB && $teamA === $teamB) {
                $teamA = competition_runner_up_id($pdo, $sourceA);
            }
            $pdo->beginTransaction();
...
) ?>"><style>.admin-shell{padding:95px 0 60px}.form-control,.form-select{background:#0b0c0e;border-color:#343941}.admin-form{padding:1.25rem}.admin-form h2{font:800 1.5rem 'Barlow Condensed',sans-serif;text-transform:uppercase}.nav-pills .nav-link.active{background:#d71920}.editor-role #tab-campeonatos,.editor-role #tab-times,.editor-role #tab-titulos,.editor-role #tab-videos,.editor-role #tab-configuracoes,.editor-role #tab-usuarios,.editor-role #tab-extra .col-lg-5>form:nth-of-type(2){display:none!important}.table-responsive.admin-list-loading{position:relative;min-height:150px;overflow:hidden}.admin-list-loader{position:absolute;inset:0;z-index:5;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:10px;background:rgba(11,13,16,.88);backdrop-filter:blur(2px);color:#fff;font:800 .72rem 'Inter',sans-serif;letter-spacing:.12em}.admin-list-loader .site-loading-spinner{width:30px;height:30px}</style></head><body class="<?= account_is_editor()
    ? "editor-role"
