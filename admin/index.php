...
<nav class="navbar fixed-top navbar-dark"><div class="container-fluid px-3 px-lg-4"><a class="navbar-brand" href="../index.php"><img class="brand-mark d-inline-block me-2" src="../assets/img/logo-season3.webp?v=5" alt="Vascao Season 3"> PAINEL S3</a><div class="d-flex align-items-center gap-3"><span class="text-secondary d-none d-md-inline">Olá, <?= e($_SESSION["conta_nome"] ?? "") ?></span><button type="button" class="btn btn-outline-light btn-sm admin-menu-toggle" aria-label="Abrir menu do painel" aria-expanded="false">Menu</button></div></div></nav>
...
<nav class="admin-navigation mb-4" aria-label="Navegação administrativa"><div class="admin-menu-groups" role="tablist" aria-label="Categorias do painel"><?php if (account_is_master()): ?><button type="button" class="admin-menu-group" data-admin-group="overview">Visão geral</button><?php endif; ?><button type="button" class="admin-menu-group active" data-admin-group="competition">Competição</button><?php if (account_is_master()): ?><button type="button" class="admin-menu-group" data-admin-group="people">Pessoas &amp; clubes</button><?php endif; ?><button type="button" class="admin-menu-group" data-admin-group="content">Conteúdo &amp; site</button></div><?php if (account_is_master()): ?><ul class="nav nav-pills admin-submenu" data-admin-submenu="overview"><li><button class="nav-link" data-bs-target="#tab-dashboard"><span aria-hidden="true" class="me-2">▦</span>Dashboard</button></li></ul><?php endif; ?><ul class="nav nav-pills admin-submenu active" data-admin-submenu="competition"><li><button class="nav-link active" data-bs-target="#tab-jogos">Pontos corridos</button></li><li><button class="nav-link" data-bs-target="#tab-mata">Mata-mata</button></li><li><button class="nav-link" data-bs-target="#tab-sumula">Importar súmula</button></li><?php if (
    account_is_master()
): ?><li><button class="nav-link" data-bs-target="#tab-campeonatos">Campeonatos</button></li><li><button class="nav-link" data-bs-target="#tab-supercopa">Supercopa</button></li><?php endif; ?><li><button class="nav-link" data-bs-target="#tab-extra">Artilharia</button></li><?php if (account_is_master()): ?><li><button class="nav-link" data-bs-target="#tab-sorteador">Sorteador</button></li><?php endif; ?></ul><?php if (account_is_master()): ?><ul class="nav nav-pills admin-submenu" data-admin-submenu="people"><li><button class="nav-link" data-bs-target="#tab-times">Técnicos e times</button></li><li><button class="nav-link" data-bs-target="#tab-usuarios">Usuários</button></li><li><button class="nav-link" data-bs-target="#tab-mercado">Mercado</button></li><li><button class="nav-link" data-bs-target="#tab-titulos">Títulos</button></li></ul><?php endif; ?><ul class="nav nav-pills admin-submenu" data-admin-submenu="content"><?php if (
    account_is_master()
): ?><li><button class="nav-link" data-bs-target="#tab-videos">Vídeos</button></li><li><button class="nav-link" data-bs-target="#tab-configuracoes">Configurações</button></li><?php endif; ?><li><button class="nav-link" data-bs-target="#tab-noticias">Notícias</button></li><li><a class="nav-link" href="../index.php" target="_blank" rel="noopener">Abrir site</a></li></ul><div class="admin-sidebar-account"><div class="admin-sidebar-shield"><?php if (!empty($adminProfile['escudo_url'])): ?><img src="<?= e($adminProfile['escudo_url']) ?>" alt="Escudo do <?= e($adminProfile['time_nome']) ?>"><?php else: ?><?= e($adminProfile['sigla'] ?: 'S3') ?><?php endif; ?></div><div><small>CONTA ADMINISTRATIVA</small><strong><?= e($_SESSION['conta_nome'] ?? 'Usuário') ?></strong><?php if (!empty($adminProfile['time_nome'])): ?><span class="text-secondary small"><?= e($adminProfile['time_nome']) ?></span><?php endif; ?></div><a href="../logout.php">Sair da conta</a></div></nav>
