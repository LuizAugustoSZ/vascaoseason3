...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    try {
        ...
        if ($action === "criar_supercopa") {
            $name = mb_substr(trim((string) ($_POST["nome"] ?? "")), 0, 150);
            $identitySelection = (string)($_POST['identidade_id'] ?? '');
            $createNewIdentity = $identitySelection === 'nova';
            $identityId = $createNewIdentity ? 0 : (int)$identitySelection;
            ...
            if (!$createNewIdentity && $identityId <= 0) {
            ...
            if (!$createNewIdentity && $identityId <= 0) throw new RuntimeException('Selecione um campeonato padrão para esta edição.');
            ...
            // Sorteia uma única vez e persiste qual vice será usado se houver campeão repetido.
            $sameChampionRule = random_int(0, 1) === 0 ? 'vice_origem_a' : 'vice_origem_b';
            ...
            if ($createNewIdentity) {
                $identityKey = competition_identity_key($name);
                if ($identityKey === '') throw new RuntimeException('Informe um nome válido para a nova competição.');
                $identityStmt = $pdo->prepare('SELECT id FROM competicao_identidades WHERE chave=? LIMIT 1');
                $identityStmt->execute([$identityKey]);
                if ($identityStmt->fetchColumn()) throw new RuntimeException('Já existe um campeonato padrão com esse nome. Selecione-o para criar a próxima edição.');
                $pdo->prepare('INSERT INTO competicao_identidades(chave,nome) VALUES(?,?)')->execute([$identityKey, $name]);
                $identityId = (int)$pdo->lastInsertId();
            }
...
function admin_nav_icon(string $name): string
{
    $icons = [
        ...
        'supercopa' => 'medal',
        'artilharia' => 'users',
        'sorteador' => 'sliders-horizontal',
        'times' => 'shield',
        'usuarios' => 'user',
        'mercado' => 'chart-no-axes-column-increasing',
        'titulos' => 'medal',
        'videos' => 'play',
        'configuracoes' => 'settings',
        'noticias' => 'newspaper',
        'regulamentos' => 'file-text',
        'site' => 'external-link',
    ];
...
@media(min-width:768px){.admin-navigation{transition:width .28s cubic-bezier(.22,1,.36,1),padding .28s cubic-bezier(.22,1,.36,1)}.admin-nav-collapsed .admin-side-nav{overflow-x:hidden;overflow-y:auto;scrollbar-gutter:stable}.admin-nav-collapsed .admin-nav-group li{position:relative}.admin-nav-collapsed .admin-nav-group .nav-link,.admin-nav-collapsed .admin-nav-group .nav-link:hover{position:relative;align-items:center;border:1px solid transparent;overflow:visible}.admin-nav-collapsed .admin-nav-group .nav-link span{position:fixed;z-index:1062;top:var(--admin-flyout-top,-100px);left:51px;display:flex;align-items:center;width:max-content;min-width:132px;height:40px;padding:0 16px 0 18px;border:1px solid #343943;border-left:0;border-radius:0 9px 9px 0;background:linear-gradient(100deg,#202329,#17191e);box-shadow:10px 8px 26px rgba(0,0,0,.34);color:#f4f5f7;opacity:0;pointer-events:none;transform:translateX(-12px) scaleX(.94);transform-origin:left center;transition:opacity .16s ease,transform .24s cubic-bezier(.22,1,.36,1)}.admin-nav-collapsed .admin-nav-group .nav-link:hover,.admin-nav-collapsed .admin-nav-group .nav-link:focus-visible{z-index:3;border-color:#343943;border-right-color:transparent;border-radius:9px 0 0 9px;background:#202329}.admin-nav-collapsed .admin-nav-group .nav-link:hover span,.admin-nav-collapsed .admin-nav-group .nav-link:focus-visible span{display:flex;opacity:1;transform:translateX(0) scaleX(1)}.admin-nav-collapsed .admin-nav-group .nav-link.active:hover,.admin-nav-collapsed .admin-nav-group .nav-link.active:focus-visible,.admin-nav-collapsed .admin-nav-group .nav-link.active:hover span,.admin-nav-collapsed .admin-nav-group .nav-link.active:focus-visible span{border-color:rgba(215,25,32,.55);background:linear-gradient(100deg,#25191c,#18191d)}.admin-side-profile{cursor:pointer}.admin-nav-collapsed .admin-side-profile-text{position:fixed;z-index:1063;top:var(--admin-profile-top,78px);left:77px;width:230px;min-width:230px;padding:12px 14px;border:1px solid #343943;border-radius:10px;background:linear-gradient(160deg,#191b21,#0e0f13);box-shadow:10px 12px 32px rgba(0,0,0,.42);opacity:0;visibility:hidden;pointer-events:none;transform:translateX(-10px) scale(.97);transform-origin:left center;transition:opacity .16s ease,transform .22s cubic-bezier(.22,1,.36,1),visibility .2s}.admin-nav-collapsed .admin-side-profile:hover .admin-side-profile-text,.admin-nav-collapsed .admin-side-profile:focus-visible .admin-side-profile-text,.admin-nav-collapsed.admin-profile-pinned .admin-side-profile-text{opacity:1;visibility:visible;pointer-events:auto;transform:none}}
...
<?php public_navbar('admin', false, true); ?>
<link rel="stylesheet" href="../assets/css/admin-navigation.css?v=<?=filemtime(__DIR__.'/../assets/css/admin-navigation.css')?>">
...
<section id="tab-jogos" class="tab-pane fade show active"><div class="row g-4"><div class="col-lg-5"><form id="form-partida" class="panel admin-form" method="post"><h2>Registrar ou editar partida</h2><input type="hidden" name="partida_id" value=""><div id="partida-edicao" class="alert alert-info d-none justify-content-between align-items-center"><span></span><button type="button" class="btn btn-sm btn-outline-info cancelar-edicao" data-form="form-partida">Cancelar edição</button></div><input type="hidden" name="csrf" value="<?= e(
    ...
    $g["status"],
) ?></td><td><div class="d-flex gap-2"><button type="button" class="btn btn-sm btn-outline-light editar-partida d-inline-flex align-items-center gap-1" data-id="<?= $g[
    "id"
    ...
    "id"
] ?>"><button class="btn btn-sm btn-outline-danger d-inline-flex align-items-center gap-1"><i data-lucide="trash-2" class="action-icon"></i> Apagar</button></form></div></td></tr><?php endforeach; ?></tbody></table></div></div></div></div></section>
<section id="tab-mata" class="tab-pane fade"><form id="form-mata" class="panel admin-form" method="post"><h2>Registrar ou editar mata-mata</h2><input type="hidden" name="jogo_mata_id" value=""><div id="mata-edicao" class="alert alert-info d-none justify-content-between align-items-center"><span></span><button type="button" class="btn btn-sm btn-outline-info cancelar-edicao" data-form="form-mata">Cancelar edição</button></div><input type="hidden" name="csrf" value="<?= e(
    ...
    $g["status"],
) ?></td><td><div class="d-flex gap-2"><button type="button" class="btn btn-sm btn-outline-light editar-mata d-inline-flex align-items-center gap-1" data-id="<?= $g[
    "id"
    ...
    "id"
] ?>"><button class="btn btn-sm btn-outline-danger d-inline-flex align-items-center gap-1"><i data-lucide="trash-2" class="action-icon"></i> Apagar</button></form></div></td></tr><?php endforeach; ?></tbody></table></div></div></section>
...
<section id="tab-times" class="tab-pane fade"><div class="row g-4"><div class="col-lg-6"><form id="form-participante" class="panel admin-form" method="post"><h2 id="participante-form-title">Novo técnico e time</h2><input type="hidden" name="participante_id" value=""><div id="participante-edicao" class="alert alert-info d-none justify-content-between align-items-center"><span></span><button type="button" class="btn btn-sm btn-outline-info cancelar-participante">Cancelar edição</button></div><input type="hidden" name="csrf" value="<?= e(
    ...
    $participant["sigla"],
) ?></td><td><button type="button" class="btn btn-sm btn-outline-light editar-participante d-inline-flex align-items-center gap-1" data-id="<?= $participant[
    "id"
] ?>"><i data-lucide="pencil" class="action-icon"></i> Editar</button></td></tr><?php endforeach; ?></tbody></table></div></div></div></div><script id="participants-admin-data" type="application/json"><?= json_encode(
    $participantsAdmin,
    JSON_HEX_TAG |
        JSON_HEX_AMP |
        JSON_HEX_APOS |
...
<section id="tab-extra" class="tab-pane fade"><div class="row g-4"><div class="col-lg-5"><form id="form-artilharia" class="panel admin-form" method="post"><h2>Registrar ou editar artilheiro</h2><input type="hidden" name="csrf" value="<?= e(
    ...
    "gols"
] ?></strong></td><td><button type="button" class="btn btn-sm btn-outline-light editar-artilheiro d-inline-flex align-items-center gap-1" data-id="<?= $scorer[
    "id"
    ...
    "gols"
] ?>"><i data-lucide="pencil" class="action-icon"></i> Editar</button></td></tr><?php endforeach; ?></tbody></table></div><div id="artilheiros-admin-empty" class="text-center text-secondary py-4 d-none">Nenhum artilheiro neste campeonato.</div></div></div></div></section>
