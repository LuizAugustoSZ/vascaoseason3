...
$participante = (int)($_SESSION['participante_id'] ?? 0);
...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (($_POST['action'] ?? '') === 'confirmar') {
            ...
            $pdo->commit();
            header('Location: mercado.php?campeonato_id=' . $campeonato);
            exit;
        }
    }
} catch (Throwable $e) {
...
<body><?php public_navbar('mercado'); ?><main class="container market-page"><a href="mercado.php?campeonato_id=<?= $campeonato ?>" class="text-secondary text-decoration-none">← Voltar</a>
        ...
        <p class="text-secondary">Cole a lista copiada do DreamTeam. Emojis e identificadores das cartas serão ignorados.</p><?php if ($erro): ?><div class="alert alert-danger"><?= e($erro) ?></div><?php endif; ?><form method="post" class="panel p-4 mb-4"><input type="hidden" name="csrf" value="<?= e(csrf_token()) ?>"><input type="hidden" name="campeonato_id" value="<?= $campeonato ?>"><input type="hidden" name="action" value="preview"><label class="form-label">Lista completa do elenco</label><textarea class="form-control" name="texto" rows="14" required><?= e($texto) ?></textarea><button class="btn btn-danger mt-3">Analisar lista</button></form><?php if ($preview): ?><section class="panel p-4">
                ...
                <form method="post" class="mt-4"><input type="hidden" name="csrf" value="<?= e(csrf_token()) ?>"><input type="hidden" name="campeonato_id" value="<?= $campeonato ?>"><input type="hidden" name="action" value="confirmar"><textarea class="d-none" name="texto"><?= e($texto) ?></textarea>
