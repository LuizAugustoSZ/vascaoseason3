...
<body><?php public_navbar('mercado'); ?><main class="container market-page" data-market-editable="<?= $podeEditarMercado ? '1' : '0' ?>"><span class="eyebrow"><?= $isMasterManagement ? 'Gestão Master' : 'Gestão do clube' ?></span>
        ...
        <?php if (!$participantId): ?><div class="panel p-4">A conta precisa estar associada a um time.</div><?php elseif ($clube && !(bool)($clube['cofre_configurado'] ?? false)): ?><section class="panel p-4 market-treasury-required"><span class="eyebrow">Primeira etapa obrigatória</span><h2>INFORME O SALDO DO COFRE</h2><p>Antes de montar o elenco ou registrar qualquer movimentação, informe o valor atual do cofre. O saldo pode ser zero, mas precisa ser confirmado pelo responsável.</p><a class="btn btn-danger" href="time.php?id=<?= $participantId ?>&editar_perfil=1">Abrir perfil e informar cofre</a></section><?php elseif ($clube): ?><section class="market-summary">
                ...
                <div><small>Ciclo <?= $ciclo['ciclo'] ?></small><strong><?= $ciclo['aberto'] ? 'MERCADO LIBERADO' : 'MERCADO TRAVADO' ?></strong></div>
            </section>
            <?php if ($rodada >= 9 && $rodada <= 13): ?><div class="alert alert-warning mb-4" role="status">
                <strong>Mercado travado após a 8ª rodada.</strong> A inscrição de jogadores e as transferências permanecem bloqueadas da 9ª até a 13ª rodada. Formação, titulares e banco podem ser reorganizados normalmente. Assim que a 13ª for concluída, o mercado será liberado para a 14ª rodada. Folgas contam normalmente como rodada cumprida.
            </div><?php elseif ($rodada === 14): ?><div class="alert alert-success mb-4" role="status">
                <strong>Mercado liberado para a 14ª rodada.</strong> A 13ª rodada foi cumprida e o clube já pode alterar novamente os jogadores inscritos na competição. A escalação continua editável em qualquer rodada.
            </div><?php endif; ?>
            ...
            <section class="panel p-4 mb-4" id="elenco">
                <div class="d-flex justify-content-between">
                    ...
                    </form><?php if (false && !$montagemInicial && $ciclo['aberto']): ?>
                    <hr>
