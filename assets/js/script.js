...
function renderTitles() {
  $('#titles-grid').html(titleGroups.length ? titleGroups.map((group,index)=>{
    ...
    const firstTrophy=group.titles.find(title=>title.trofeu_url)?.trofeu_url;
    return `<div class="col-md-6 col-xl-4"><article class="title-card"><div class="title-card-head">${firstTrophy?`<img class="title-trophy-image" src="${esc(firstTrophy)}" alt="">`:'<span class="title-trophy">🏆</span>'}<div><small>Técnico</small><h3>${esc(group.coach)}</h3>${group.team ? `<p>${group.participantId?teamLink(group.participantId,group.team):esc(group.team)}</p>`: ''}</div><strong>${group.titles.length}</strong></div><div class="title-list">${visible.map(t=>`<div>${t.trofeu_url?`<img src="${esc(t.trofeu_url)}" alt="">`:''}<span>${esc(t.titulo)}</span><b>${esc(t.temporada)}</b></div>`).join('')}</div>${pagination}</article></div>`;
...
function renderSite(data) {
  ...
  const leagueChampionId=Number(data.titulo_liga?.campeao_id || 0),earlyChampion=Boolean(data.titulo_liga?.campeao_antecipado);
  $('#standings-body').html(data.classificacao.length ? data.classificacao.map(t=>{const champion=Number(t.id)===leagueChampionId,libertadores=Number(t.posicao)<=4,sulamericana=Number(t.posicao)>=5&&Number(t.posicao)<=8,label=champion?(earlyChampion?'CAMPEÃO ANTECIPADO':'CAMPEÃO'):'';return `<tr class="standings-team-row${champion?' is-champion':''}${libertadores?' is-libertadores':''}${sulamericana?' is-sulamericana':''}" tabindex="0" role="link" data-team-href="time.php?id=${Number(t.id)}"><td><span class="position-pill ${libertadores?'top':sulamericana?'sulamericana':''}">${t.posicao}</span></td><td><div class="team-cell">${badge(t)}<span>${esc(t.time_nome)}${label?`<small class="standings-champion-label">${label}</small>`:''}</span></div></td><td>${esc(t.nome)}</td><td><strong>${t.pts}</strong></td><td>${t.j}</td><td>${t.v}</td><td>${t.e}</td><td>${t.d}</td><td>${t.sg>0?'+':''}${t.sg}</td></tr>`}).join(''): `<tr><td colspan="9">${publicEmpty('A classificação será exibida assim que a competição começar.')}</td></tr>`);
...
async function downloadCompetition(type){
  ...
  const target=type==='mata'?document.querySelector(currentChampionship?.tipo==='supercopa'?'#supercopa .panel':'#mata-mata .panel'):document.querySelector('#pontos-corridos .panel');const exportArea=document.createElement('div');exportArea.style.cssText='position:fixed;left:-10000px;top:0;width:1400px;padding:40px;background:#101318;color:#fff;z-index:-1';const competitionLogo=currentChampionship?.logo_url?`<img src="${esc(currentChampionship.logo_url)}" alt="Logo ${esc(currentChampionship?.nome||'Campeonato')}" style="display:block;width:190px;height:128px;object-fit:contain;object-position:center">`:'';exportArea.innerHTML=`<header style="display:flex;align-items:center;justify-content:space-between;gap:32px;margin-bottom:24px"><div><h1 style="margin:0 0 24px;font-weight:800">${esc(currentChampionship?.nome||'Campeonato')}</h1><p style="margin:0;color:#9ca8bd">${currentChampionship?.status==='finalizado'?'CAMPEONATO FINALIZADO':'CAMPEONATO EM ANDAMENTO'}</p></div>${competitionLogo}</header>`;const clone=target.cloneNode(true);clone.querySelectorAll('.competition-download').forEach(item=>item.remove());exportArea.appendChild(clone);document.body.appendChild(exportArea);await Promise.all([...exportArea.querySelectorAll('img')].map(img=>img.complete?Promise.resolve():new Promise(resolve=>{img.onload=img.onerror=resolve})));
...
$(function(){
  $('.competition-tabs').before('<div class="panel competition-identity-panel p-3 mb-3"><img id="championship-identity" class="d-none" src="" alt=""><div><label class="form-label small">CAMPEONATO</label><select id="championship-select" class="form-select"></select><small id="championship-title" class="text-secondary"></small></div></div>');
