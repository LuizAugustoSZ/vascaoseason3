...
function renderLeagueGames() {
  ...
  $('#league-games').html(visibleGames.length ? `<div class="game-sides-head"><span>MANDANTE</span><i aria-hidden="true"></i><span>VISITANTE</span></div>${visibleGames.map(g=>`<div class="game-item"><div class="game-meta"><span>Rodada ${g.rodada}</span><span>${g.status}</span></div><div class="game-score"><span>${esc(g.mandante)}<small>${esc(g.tecnico_mandante)}</small></span><b class="score">${g.gols_mandante ?? '-'} × ${g.gols_visitante ?? '-'}</b><span>${esc(g.visitante)}<small>${esc(g.tecnico_visitante)}</small></span></div></div>`).join('')}` : publicEmpty(leagueGames.length?'Nenhuma partida corresponde à sua busca.':'O calendário de jogos será divulgado em breve.'));
...
function renderSite(data) {
  ...
  $('#standings-body').html(data.classificacao.length ? data.classificacao.map(t=>`<tr><td><span class="position-pill ${t.posicao<=4?'top':''}">${t.posicao}</span></td><td><div class="team-cell">${badge(t)}<span>${esc(t.time_nome)}</span></div></td><td>${esc(t.nome)}</td><td><strong>${t.pts}</strong></td><td>${t.j}</td><td>${t.v}</td><td>${t.e}</td><td>${t.d}</td><td>${t.sg>0?'+':''}${t.sg}</td></tr>`).join('') : `<tr><td colspan="9">${publicEmpty('A classificação será exibida assim que a competição começar.')}</td></tr>`);
  // Monta os cards dos participantes.
  $('#participants-grid').html(data.participantes.length ? data.participantes.map(t=>`<div class="col-6 col-lg-3"><article class="participant-card">${badge(t)}<h3>${esc(t.time_nome)}</h3><p class="coach-name">Técnico: ${esc(t.nome)}</p><p>${esc(t.descricao || 'Participante da Season 3')}</p></article></div>`).join('') : publicEmpty('Os participantes da temporada serão apresentados em breve.'));
  ...
  const titlesByCoach = (data.titulos || []).reduce((groups, title) => { const coach=title.tecnico || 'Técnico não informado'; const team=title.time_nome || ''; const key=JSON.stringify([coach,team]); (groups[key] ||= []).push(title); return groups; }, {});
  $('#titles-grid').html(Object.keys(titlesByCoach).length ? Object.entries(titlesByCoach).map(([key,titles])=>{const [coach,team]=JSON.parse(key);return `<div class="col-md-6 col-xl-4"><article class="title-card"><div class="title-card-head"><span class="title-trophy">🏆</span><div><small>Técnico</small><h3>${esc(coach)}</h3>${team ? `<p>${esc(team)}</p>` : ''}</div><strong>${titles.length}</strong></div><div class="title-list">${titles.map(t=>`<div><span>${esc(t.titulo)}</span><b>${esc(t.temporada)}</b></div>`).join('')}</div></article></div>`}).join('') : publicEmpty('A história dos campeões será apresentada em breve.'));
