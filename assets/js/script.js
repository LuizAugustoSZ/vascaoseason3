...
const bracketSide=(game,side)=>{const team=game[`time_${side}`],id=game[`time_${side}_id`];if(team)return `<a class="bracket-team-shield" href="time.php?id=${Number(id)}" aria-label="Abrir página de ${esc(team)}">${badge({escudo_url:game[`escudo_${side}`],sigla:game[`sigla_${side}`],nome:game[`tecnico_${side}`],time_nome:team})}</a><span>${teamLink(id,team)}</span>`;const phase=game[`origem_${side}_fase`],order=game[`origem_${side}_ordem`],origin=game[`origem_${side}_tipo`]==='perdedor'?'Perdedor':'Vencedor';return `<span class="team-badge bracket-wait">?</span><span class="bracket-placeholder">${origin} de ${esc(phase || 'fase anterior')} ${order?esc(order):''}</span>`;};
