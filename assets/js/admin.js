...
if(adminNavigation)adminNavigation.insertAdjacentHTML('afterbegin','<button type="button" class="admin-sidebar-collapse" aria-label="Expandir ou recolher menu">‹</button>');
const adminSidebarCollapse=document.querySelector('.admin-sidebar-collapse');
if(adminMenuToggle&&adminNavigation){if(innerWidth>=992&&localStorage.getItem('admin-nav-collapsed')==='1')document.body.classList.add('admin-nav-collapsed');adminMenuToggle.addEventListener('click',()=>{if(innerWidth>=992){const collapsed=document.body.classList.toggle('admin-nav-collapsed');localStorage.setItem('admin-nav-collapsed',collapsed?'1':'0');adminMenuToggle.setAttribute('aria-expanded',String(!collapsed));return}const open=document.body.classList.toggle('admin-nav-open');adminMenuToggle.setAttribute('aria-expanded',String(open))});adminNavigation.addEventListener('click',event=>{if(innerWidth<992&&event.target.closest('button,a')){document.body.classList.remove('admin-nav-open');adminMenuToggle.setAttribute('aria-expanded','false')}});document.addEventListener('keydown',event=>{if(event.key==='Escape'&&innerWidth<992){document.body.classList.remove('admin-nav-open');adminMenuToggle.setAttribute('aria-expanded','false')}})}
if(adminSidebarCollapse)adminSidebarCollapse.addEventListener('click',()=>{const collapsed=document.body.classList.toggle('admin-nav-collapsed');localStorage.setItem('admin-nav-collapsed',collapsed?'1':'0');adminMenuToggle?.setAttribute('aria-expanded',String(!collapsed))});
