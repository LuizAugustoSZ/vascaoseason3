(function(){
// Mantém busca, filtros e página atual enquanto apenas os dados da aba são renovados.
const adminListState=window.__adminListState||(window.__adminListState={});
const adminListKey=table=>{const pane=table?.closest('.tab-pane');if(!pane)return'';return `${pane.id}:${[...pane.querySelectorAll('table')].indexOf(table)}`;};
const adminPageFrom=pagination=>Number((pagination?.textContent.match(/Página\s+(\d+)/i)||[])[1]||1);
function captureAdminListState(pane){
  pane?.querySelectorAll('table').forEach(table=>{const wrapper=table.closest('.table-responsive'),controls=wrapper?.previousElementSibling,pagination=wrapper?.nextElementSibling,key=adminListKey(table);if(!key)return;const values=[...controls?.querySelectorAll('input,select')||[]].map(control=>control.value);adminListState[key]={page:adminPageFrom(pagination),values};});
  const scorerTable=pane?.querySelector('#artilheiros-admin-body')?.closest('table');if(scorerTable){const key=adminListKey(scorerTable),values={};['artilheiros-admin-filter','artilheiros-admin-search','artilheiros-admin-team'].forEach(id=>{const control=document.getElementById(id);if(control)values[id]=control.value});const pagination=document.getElementById('artilheiros-admin-empty')?.nextElementSibling;adminListState[key]={page:adminPageFrom(pagination),values};}
}
function showAdminListLoading(pane){pane?.querySelectorAll('.table-responsive').forEach(wrapper=>{wrapper.classList.add('admin-list-loading');wrapper.setAttribute('aria-busy','true');if(!wrapper.querySelector('.admin-list-loader'))wrapper.insertAdjacentHTML('beforeend','<div class="admin-list-loader" role="status"><span class="site-loading-spinner"></span><strong>ATUALIZANDO LISTAGEM</strong></div>')});}
function hideAdminListLoading(pane){pane?.querySelectorAll('.table-responsive.admin-list-loading').forEach(wrapper=>{wrapper.classList.remove('admin-list-loading');wrapper.removeAttribute('aria-busy');wrapper.querySelector('.admin-list-loader')?.remove()});}
const adminMenuToggle=document.querySelector('.admin-menu-toggle'),adminNavigation=document.querySelector('.admin-navigation');
const adminSidebarCollapse=document.querySelector('.admin-sidebar-collapse');
if(!window.__adminShellEventsBound){
window.__adminShellEventsBound=true;
if(adminMenuToggle&&adminNavigation){adminMenuToggle.addEventListener('click',()=>{const open=document.body.classList.toggle('admin-nav-open');adminMenuToggle.setAttribute('aria-expanded',String(open))});adminNavigation.addEventListener('click',event=>{if(innerWidth<768&&event.target.closest('[data-bs-target],a')){document.body.classList.remove('admin-nav-open');adminMenuToggle.setAttribute('aria-expanded','false')}});document.addEventListener('keydown',event=>{if(event.key==='Escape'&&innerWidth<768){document.body.classList.remove('admin-nav-open');adminMenuToggle.setAttribute('aria-expanded','false')}})}
if(adminSidebarCollapse)adminSidebarCollapse.addEventListener('click',()=>{if(innerWidth<768){document.body.classList.remove('admin-nav-open');adminMenuToggle?.setAttribute('aria-expanded','false');return}const collapsed=document.body.classList.toggle('admin-nav-collapsed');try{sessionStorage.setItem('admin-sidebar-state',collapsed?'collapsed':'expanded')}catch(error){}adminSidebarCollapse.setAttribute('aria-label',collapsed?'Expandir menu':'Recolher menu')});
document.querySelectorAll('.admin-side-nav .nav-link').forEach(button=>{const label=button.querySelector('span')?.textContent?.trim();if(label){button.setAttribute('aria-label',label);button.removeAttribute('title')}const placeFlyout=()=>button.style.setProperty('--admin-flyout-top',`${button.getBoundingClientRect().top}px`);button.addEventListener('pointerenter',placeFlyout);button.addEventListener('focus',placeFlyout)});
const adminProfile=document.querySelector('.admin-side-profile');
if(adminProfile){const setPinned=pinned=>{document.body.classList.toggle('admin-profile-pinned',pinned);adminProfile.setAttribute('aria-expanded',String(pinned))};const togglePinned=event=>{event.stopPropagation();setPinned(!document.body.classList.contains('admin-profile-pinned'))};adminProfile.addEventListener('pointerenter',()=>adminProfile.style.setProperty('--admin-profile-top',`${adminProfile.getBoundingClientRect().top}px`));adminProfile.addEventListener('click',togglePinned);adminProfile.addEventListener('keydown',event=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();togglePinned(event)}});document.addEventListener('click',()=>setPinned(false));document.addEventListener('keydown',event=>{if(event.key==='Escape')setPinned(false)})}
document.querySelectorAll('.admin-side-nav [data-bs-target]').forEach(button=>button.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();const target=document.querySelector(button.dataset.bsTarget);if(!target)return;document.querySelectorAll('.admin-side-nav [data-bs-target]').forEach(item=>{item.classList.remove('active');item.setAttribute('aria-selected','false')});document.querySelectorAll('.tab-content>.tab-pane').forEach(pane=>pane.classList.remove('show','active'));button.classList.add('active');button.setAttribute('aria-selected','true');target.classList.add('show','active');const tab=target.id.replace(/^tab-/,'');const url=new URL(location.href);url.searchParams.set('tab',tab);history.replaceState(null,'',url)}));
}
document.querySelectorAll('[data-admin-frame]:not([data-admin-frame-bound])').forEach(frame=>{frame.dataset.adminFrameBound='1';frame.addEventListener('load',()=>{try{frame.style.height=`${Math.max(900,frame.contentDocument.documentElement.scrollHeight)}px`}catch(error){}})});
...
function setupLeagueAdminTable(perPage=5){
  ...
  const saved=adminListState[adminListKey(table)]||{};[search,teamFilter,roundFilter].forEach((control,index)=>{if(saved.values?.[index]!==undefined)control.value=saved.values[index]});let page=saved.page||1;
...
if(scorerFilter && scorerForm){
  ...
  const pagination=document.createElement('div');pagination.className='d-flex justify-content-between align-items-center p-3 border-top';document.getElementById('artilheiros-admin-empty').after(pagination);const saved=adminListState[adminListKey(document.querySelector('#artilheiros-admin-body')?.closest('table'))]||{};if(saved.values){scorerFilter.value=saved.values['artilheiros-admin-filter']||'';search.value=saved.values['artilheiros-admin-search']||'';teamSelect.value=saved.values['artilheiros-admin-team']||'';}let page=saved.page||1;
...
function setupUniversalAdminLists(){
  document.querySelectorAll('.tab-pane table').forEach(table=>{
    ...
    wrapper.before(controls);const pagination=document.createElement('div');pagination.className='d-flex justify-content-between align-items-center gap-2 p-3 border-top';wrapper.after(pagination);const saved=adminListState[adminListKey(table)]||{};[...controls.querySelectorAll('input,select')].forEach((control,index)=>{if(saved.values?.[index]!==undefined)control.value=saved.values[index]});let page=saved.page||1;
