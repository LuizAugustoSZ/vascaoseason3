...
const siteVersions=[
['19.19','A página de Regulamento agora exibe notícias selecionadas dinamicamente, e o painel administrativo ganhou uma área própria para associar, trocar ou remover essas publicações.','Regulamentos ligados às notícias'],
...
['17.6','A Gestão da Competição agora mostra somente campeonatos ativos de pontos corridos e bloqueia o acesso a competições encerradas.','Gestão apenas de pontos corridos ativos'],
...
['14.5','A área Jogadores ganhou rankings alternáveis de artilheiros e assistências, com paginação compacta e acesso ao histórico individual.'],
['14.4','Partidas priorizam a abertura da súmula, links de clubes ficaram restritos ao nome e escudo, classificação ganhou linhas inteiras clicáveis e o visual do confronto direto e da escalação foi corrigido.'],
...
document.addEventListener('DOMContentLoaded',()=>{
 ...
 if(adminVersions.length&&adminVersions[0][0]!=='a2.3')adminVersions.unshift(['a2.3','Supercopas entre campeões ganharam vagas automáticas e as ações do painel passaram a recarregar listas, filtros e paginação sem atualizar a tela inteira.']);
 ...
 if(adminVersions.length&&adminVersions[0][0]!=='a3.0')adminVersions.unshift(['a3.0','Admin ganhou a central Campeonatos padrões e criação orgânica da próxima edição de Supercopa sem duplicar imagens.']);
 if(adminVersions.length&&!adminVersions.some(([version])=>version==='a2.9'))adminVersions.splice(1,0,['a2.9','Sorteador ganhou modelos reutilizáveis, numeração automática de edições e entrega automática da taça ao campeão.']);
 ...
 const previousReleaseVersion=previousReleaseSignature.split('|')[0]||'';
 const campaignStartVersion='17.6';
 const campaignStartIndex=siteVersions.findIndex(([version])=>version===campaignStartVersion);
 const previousReleaseIndex=siteVersions.findIndex(([version])=>version===previousReleaseVersion);
 const releaseItems=previousReleaseVersion===''&&campaignStartIndex>=0
  ? siteVersions.slice(0,campaignStartIndex+1)
  : previousReleaseIndex>0
   ? siteVersions.slice(0,previousReleaseIndex)
   : siteVersions.slice(0,1);
 const releaseSignature=`${releaseVersion}|${siteVersions[0][1]}`;
 const releaseCards=releaseItems.map(([version,description,title])=>`<article><div><h3>v${version} · ${title||'Novidades desta versão'}</h3><p>${description}</p></div></article>`).join('');
 ...
 if(!releaseSeen){
  ...
  if(document.readyState==='complete')showRelease();else window.addEventListener('load',showRelease,{once:true});
 }
});
