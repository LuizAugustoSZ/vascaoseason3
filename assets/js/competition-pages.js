...
renderSite=function(data){
 ...
 $('<div id="competition-summary" class="competition-summary"></div>').html([[data.classificacao.length,'Equipes','users'],[new Set(games.map(g=>g.rodada)).size,'Rodadas','calendar-days'],[games.length,'Jogos','swords'],[status,'Situação','activity']].map(([n,label,icon])=>`<article><span class="stat-icon" aria-hidden="true"><i data-lucide="${icon}"></i></span><div><small>${label}</small><strong>${esc(n)}</strong></div></article>`).join('')).prependTo('#pontos-corridos');
...
renderLeagueGames=function(){
 ...
 $('<section id="round-summary" class="panel round-summary"></section>').html(`<div class="panel-head"><h3><i data-lucide="chart-no-axes-column-increasing" aria-hidden="true"></i>Resumo ${round==='all'?'dos jogos':'da rodada '+esc(round)}</h3></div>${done.length?`<div class="round-summary-grid">${[[goals,'Gols marcados','goal'],[(goals/done.length).toLocaleString('pt-BR',{maximumFractionDigits:2}),'Média por jogo','gauge'],[home,'Vitórias em casa','house'],[draw,'Empates','equal'],[done.length-home-draw,'Vitórias fora','plane']].map(([n,label,icon])=>`<article><span class="stat-icon" aria-hidden="true"><i data-lucide="${icon}"></i></span><div><small>${label}</small><strong>${n}</strong></div></article>`).join('')}</div><p>${done.length} de ${games.length} jogos finalizados</p>${best&&Number(best.gols_mandante)!==Number(best.gols_visitante)?`<div class="round-highlight"><span>Maior diferença</span><button type="button" class="round-highlight-game match-open" data-match-type="pontos" data-match-id="${Number(best.id)}"><span class="round-highlight-team">${badge({escudo_url:best.mandante_escudo,sigla:best.mandante_sigla,nome:best.mandante})}<span>${esc(best.mandante)}</span></span><b>${best.gols_mandante} × ${best.gols_visitante}</b><span class="round-highlight-team"><span>${esc(best.visitante)}</span>${badge({escudo_url:best.visitante_escudo,sigla:best.visitante_sigla,nome:best.visitante})}</span></button></div>`:''}`:'<p>Aguardando resultados nesta rodada.</p>'}`).appendTo('#pontos-corridos');
 if(window.lucide)window.lucide.createIcons();
