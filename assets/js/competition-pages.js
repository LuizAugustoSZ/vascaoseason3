...
renderLeagueGames=function(){
 ...
 $('<section id="round-summary" class="panel round-summary"></section>').html(`<div class="panel-head"><h3>Resumo ${round==='all'?'dos jogos':'da rodada '+esc(round)}</h3></div>${done.length?`<div class="round-summary-grid">${[[goals,'Gols marcados'],[(goals/done.length).toLocaleString('pt-BR',{maximumFractionDigits:2}),'Média por jogo'],[home,'Vitórias em casa'],[draw,'Empates'],[done.length-home-draw,'Vitórias fora']].map(([n,label])=>`<article><strong>${n}</strong><small>${label}</small></article>`).join('')}</div><p>${done.length} de ${games.length} jogos finalizados</p>${best&&Number(best.gols_mandante)!==Number(best.gols_visitante)?`<p>Maior diferença: ${esc(best.mandante)} <b>${best.gols_mandante} × ${best.gols_visitante}</b> ${esc(best.visitante)}</p>`:''}`:'<p>Aguardando resultados nesta rodada.</p>'}`).appendTo('#pontos-corridos .col-xl-4');
