document.addEventListener('DOMContentLoaded',()=>{
  ...
  const check=async()=>{
    try{
      ...
      control.classList.toggle('d-none',!data.changed);
      if(data.changed)control.classList.add('d-flex');
      status.textContent=data.changed?'A produção possui dados mais recentes.':'';
    }catch(error){control.classList.add('d-none');}
  };
