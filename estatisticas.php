...
$cards=[];$details=[];
$shieldsById=[];$shieldsByName=[];
foreach($clubs as $club){$shieldsById[(int)$club['id']]=$club['escudo_url'];$shieldsByName[$club['time_nome']]=$club['escudo_url'];}
$add=function(string $section,string $key,string $label,array $rows,string $nameField,string $valueField,string $suffix='',?callable $valueFormat=null,string $note='')use(&$cards,&$details,$shieldsById,$shieldsByName):void{
    $rows=array_slice($rows,0,10);$first=$rows[0]??null;if(!$first)return;
    $format=$valueFormat??static fn($v)=>(string)$v;
    $shield=$first['shield']??$first['escudo_url']??$shieldsById[(int)($first['club_id']??$first['participante_id']??0)]??$shieldsByName[$first['club']??$first['clube']??$first['name']??'']??'';
    $cards[$section][]=compact('key','label','note','shield')+['name'=>(string)($first[$nameField]??'Sem dados'),'value'=>$format($first[$valueField]??0).$suffix];
...
foreach($competitionTitles as $key=>$competition){
    ...
    if($ranking){
        ...
        if(count($leaders)>1)$cards['Títulos'][array_key_last($cards['Títulos'])]['shield']='';
    }else{
...
$statIcons=['team-wins'=>'trophy','team-games'=>'calendar-days','team-goals'=>'goal','team-defense'=>'shield-check','team-gd'=>'circle-plus','team-pct'=>'percent','titles'=>'crown','goals'=>'goal','assists'=>'handshake','contributions'=>'badge-plus','yellow'=>'square','red'=>'square','penalties'=>'circle-dot','free-kicks'=>'crosshair','biggest-win'=>'medal','most-goals'=>'chart-no-axes-column-increasing','draw-goals'=>'equal','most-played'=>'swords','dominance'=>'shield','wealth'=>'landmark','spent'=>'shopping-cart','revenue'=>'badge-dollar-sign','activity'=>'repeat-2','purchase'=>'badge-dollar-sign','sale'=>'circle-dollar-sign'];
...
<main class="statistics-page section-pad"><div class="container"><header class="statistics-hero"><div><span class="eyebrow">Tudo que já aconteceu, transformado em números</span><h1>ESTATÍSTICAS <em>&</em> RECORDES</h1><p>A história do Vascão em números.</p><small><?=count($matches)?> partidas • <?=$totalGoals?> gols • <?=$totalCompetitions?> competições • <?=count($players)?> jogadores</small></div><strong>MAIS QUE UM JOGO,<br><em>UMA HISTÓRIA.</em></strong></header>
...
<section class="statistics-global" aria-label="Resumo histórico"><article><span class="stat-icon" aria-hidden="true"><i data-lucide="calendar-check"></i></span><strong><?=count($matches)?></strong><span>partidas finalizadas</span></article><article><span class="stat-icon" aria-hidden="true"><i data-lucide="goal"></i></span><strong><?=$totalGoals?></strong><span>gols registrados</span></article><article><span class="stat-icon" aria-hidden="true"><i data-lucide="trophy"></i></span><strong><?=$totalCompetitions?></strong><span>competições no recorte</span></article><article><span class="stat-icon" aria-hidden="true"><i data-lucide="users"></i></span><strong><?=count($players)?></strong><span>jogadores com eventos</span></article><article><span class="stat-icon" aria-hidden="true"><i data-lucide="repeat-2"></i></span><strong><?=count($finance['moves'])?></strong><span>transferências</span></article><article><span class="stat-icon" aria-hidden="true"><i data-lucide="circle-dollar-sign"></i></span><strong><?=e($formatMoney($transferVolume))?></strong><span>movimentados</span></article></section>
...
<?php if($featured):?><section class="statistics-section statistics-featured" id="destaques" data-stat-section="all"><div class="statistics-section-title"><span class="d-inline-flex"><i data-lucide="sparkles" aria-hidden="true"></i></span><h2>Destaques históricos</h2><p>Os principais recordes da história do Vascão.</p></div><div class="statistics-featured-grid"><?php foreach($featured as $card):?><button class="statistics-card" type="button" data-stat-card="<?=e($card['key'])?>"><?php if(!empty($card['shield'])):?><img class="statistics-card-shield" src="<?=e($card['shield'])?>" alt="" aria-hidden="true" loading="lazy" onerror="this.hidden=true"><?php endif;?><span class="stat-icon" aria-hidden="true"><i data-lucide="<?=e($iconFor($card['key']))?>"></i></span><small><?=e($card['label'])?></small><strong><?=e($card['name'])?></strong><b><?=e($card['value'])?></b><span>Ver ranking completo →</span></button><?php endforeach;?></div></section><?php endif;?>
<?php foreach($cards as $section=>$sectionCards):$sectionId=$sectionIds[$section]??'';?><section class="statistics-section" id="<?=$sectionId?>" data-stat-section="<?=$sectionId?>" hidden><div class="statistics-section-title"><h2><?=e($section)?></h2></div><div class="statistics-grid"><?php foreach($sectionCards as $card):?><button class="statistics-card" type="button" data-stat-card="<?=e($card['key'])?>"><?php if(!empty($card['shield'])):?><img class="statistics-card-shield" src="<?=e($card['shield'])?>" alt="" aria-hidden="true" loading="lazy" onerror="this.hidden=true"><?php endif;?><span class="stat-icon" aria-hidden="true"><i data-lucide="<?=e($iconFor($card['key']))?>"></i></span><small><?=e($card['label'])?></small><strong><?=e($card['name'])?></strong><b><?=e($card['value'])?></b><span><?=e($card['note']?:'Ver ranking e detalhes')?> →</span></button><?php endforeach;?></div></section><?php endforeach;?>
