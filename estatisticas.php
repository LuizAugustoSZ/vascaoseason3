...
<?php if($featured):?><section class="statistics-section statistics-featured" id="destaques" data-stat-section="all"><div class="statistics-section-title"><span>★</span><h2>Destaques históricos</h2><p>Os principais recordes da história do Vascão.</p></div><div class="statistics-featured-grid"><?php foreach($featured as $card):?><button class="statistics-card" type="button" data-stat-card="<?=e($card['key'])?>"><?php if(!empty($card['shield'])):?><img class="statistics-card-shield" src="<?=e($card['shield'])?>" alt="" aria-hidden="true" loading="lazy" onerror="this.hidden=true"><?php endif;?><small><?=e($card['label'])?></small><strong><?=e($card['name'])?></strong><b><?=e($card['value'])?></b><span>Ver ranking completo →</span></button><?php endforeach;?></div></section><?php endif;?>
<?php foreach($cards as $section=>$sectionCards):$sectionId=$sectionIds[$section]??'';?><section class="statistics-section" id="<?=$sectionId?>" data-stat-section="<?=$sectionId?>" hidden><div class="statistics-section-title"><h2><?=e($section)?></h2></div><div class="statistics-grid"><?php foreach($sectionCards as $card):?><button class="statistics-card" type="button" data-stat-card="<?=e($card['key'])?>"><?php if(!empty($card['shield'])):?><img class="statistics-card-shield" src="<?=e($card['shield'])?>" alt="" aria-hidden="true" loading="lazy" onerror="this.hidden=true"><?php endif;?><small><?=e($card['label'])?></small><strong><?=e($card['name'])?></strong><b><?=e($card['value'])?></b><span><?=e($card['note']?:'Ver ranking e detalhes')?> →</span></button><?php endforeach;?></div></section><?php endforeach;?>
