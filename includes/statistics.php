...
function statistics_players(PDO $pdo, int $championshipId = 0, int $clubId = 0): array
{
    ...
    $players=[];
    foreach($goals as $goal){$key=mb_strtolower(trim($goal['jogador']),'UTF-8').'|'.$goal['participante_id'];if(!isset($players[$key]))$players[$key]=['name'=>$goal['jogador'],'club'=>$goal['clube'],'club_id'=>(int)$goal['participante_id'],'goals'=>0,'assists'=>0,'yellow'=>0,'red'=>0,'penalties'=>0,'free_kicks'=>0,'olympic'=>0];$players[$key]['goals']++;if($goal['tipo']==='penalti')$players[$key]['penalties']++;if($goal['tipo']==='falta')$players[$key]['free_kicks']++;if($goal['tipo']==='olimpico')$players[$key]['olympic']++;}
