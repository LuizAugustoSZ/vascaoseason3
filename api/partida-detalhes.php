...
try {
    ...
    $matches = null;
    if ($type === 'mata') {
        $legs = $pdo->prepare("SELECT j.id,j.jogo,CONCAT(j.fase,' ',j.ordem) etapa,NULL data_partida,j.status,j.gols_a,j.gols_b,a.id time_a_id,a.time_nome time_a,a.sigla sigla_a,a.escudo_url escudo_a,b.id time_b_id,b.time_nome time_b,b.sigla sigla_b,b.escudo_url escudo_b,c.nome campeonato,j.penaltis_a,j.penaltis_b
            FROM jogos_mata_mata j JOIN participantes a ON a.id=j.time_a_id JOIN participantes b ON b.id=j.time_b_id JOIN campeonatos c ON c.id=j.campeonato_id
