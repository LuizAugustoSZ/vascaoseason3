...
try {
    ...
    $matches = null;
    if ($type === 'mata') {
        $legs = $pdo->prepare("SELECT j.id,j.jogo,CONCAT(j.fase,' ',j.ordem) etapa,COALESCE(c.data_inicio,s.criado_em) data_partida,j.status,j.gols_a,j.gols_b,a.id time_a_id,a.time_nome time_a,a.sigla sigla_a,a.escudo_url escudo_a,b.id time_b_id,b.time_nome time_b,b.sigla sigla_b,b.escudo_url escudo_b,c.nome campeonato,j.penaltis_a,j.penaltis_b
