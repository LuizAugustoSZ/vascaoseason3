...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    }else{$pdo->prepare('UPDATE notifications SET read_at=NOW() WHERE account_id=? AND read_at IS NULL AND id<=?')->execute([$account,max(0,(int)($_POST['through']??0))]);}
}
$q=$pdo->prepare('SELECT id,title,body,url,created_at,read_at FROM notifications WHERE account_id=? ORDER BY id DESC LIMIT 30');$q->execute([$account]);$items=$q->fetchAll();
